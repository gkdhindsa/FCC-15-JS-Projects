const text = [
  ...
  const generated_text=document.querySelector('.text')
  ...
  form.addEventListener('submit', function(e){
    ...
    if(isNaN(value) ||  value < 0 || value>9){
        generated_text.innerHTML=`<p class="result"> ${text[random]}</p>`;
    }
    else{
        let tempText=text.slice(0, value);
        let temp=''
        for(let i=0;i<value;i++){
            temp+=`<p class="result"> ${text[i]}</p>`
            generated_text.innerHTML=temp
        }
